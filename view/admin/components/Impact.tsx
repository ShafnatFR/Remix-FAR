...
export const Impact: React.FC<ImpactProps> = ({ claims = [] }) => {
    ...
    const useMemoResult = useMemo(() => {
        ...
        completedClaims.forEach(c => {
            ...
            waterSum += (c.socialImpact?.waterSaved || 0);
            landSum += (c.socialImpact?.landSaved || 0);
        ...
        return {
            ...
            totalWaterSaved: waterSum,
            totalLandSaved: landSum,
            uniqueBeneficiaries: uniqueReceivers.size,
            chartData: { waste: wasteData, social: socialData, co2: co2Data, labels }
        };
    }, [claims, filter]);

    const { totalSavedKg, totalCo2Saved, totalWaterSaved, totalLandSaved, uniqueBeneficiaries, chartData } = useMemoResult;

    const TARGET_KG = 50000;
    const targetPercentage = Math.max(0.5, Math.min(100, parseFloat(((totalSavedKg / TARGET_KG) * 100).toFixed(1))));
                ...
                <div className="flex flex-wrap items-center gap-3 bg-stone-100 dark:bg-stone-800 p-1.5 rounded-2xl border border-stone-200 dark:border-stone-700">
                    {(['harian', 'bulanan', 'tahunan'] as TimeFilter[]).map((t) => (
                        <button
                            key={t}
                            onClick={() => setFilter(t)}
                            className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${filter === t
                ...
                <div className="grid grid-cols-1 xl:grid-cols-12 gap-12 items-center relative z-10">
                    ...
                    <div className="xl:col-span-4 space-y-8">
                        ...
                        <div className="bg-white/5 border border-white/10 p-6 rounded-3xl backdrop-blur-md">
                            <p className="text-[9px] font-black text-orange-500 tracking-[0.2em] uppercase mb-4">Misi Keberlanjutan</p>
                            <p className="text-xs text-stone-400 leading-relaxed font-medium">Fokus pada pengurangan limbah organik secara sistematis untuk mendukung target Net Zero Emission 2050.</p>
                        </div>
                    </div>
                        ...
                        <div className="flex flex-col items-center justify-center border-l border-white/10 pl-0 md:pl-8">
                            <div className="w-56 h-56 relative animate-in zoom-in-75 duration-1000">
                                ...
                                <div className="absolute inset-0 flex flex-col items-center justify-center">
                                    <span className="text-6xl font-black text-white italic tracking-tighter leading-none">{targetPercentage}%</span>
                                    <span className="text-[9px] font-black text-orange-500 uppercase tracking-widest mt-2 px-3 py-1 bg-orange-500/10 rounded">Achieved</span>
                                </div>
                            </div>
                            <div className="mt-8 text-center space-y-2">
                                <p className="text-stone-300 text-lg font-black italic tracking-tight">
                                    {totalSavedKg.toLocaleString('id-ID', { maximumFractionDigits: 1 })} <span className="text-stone-500 text-[10px] not-italic mr-2">KG</span>
                                    <span className="text-stone-600 text-[10px] mx-2">OF</span>
                                    <span className="text-orange-500">50,000</span> <span className="text-stone-500 text-[10px] not-italic">KG TARGET</span>
                                </p>
                                <p className="text-[9px] font-bold text-stone-500 uppercase tracking-[0.2em]">
                                    DIAGRAM PENCAPAIAN ESG TAHUNAN
                                </p>
                            </div>
                        </div>
