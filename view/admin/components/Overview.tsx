...
export const Overview: React.FC<OverviewProps> = ({ onNavigate, stats, claims = [], inventory = [], users = [] }) => {
    ...
    // Calculate derived metrics based on real claim data
    const { totalKgSaved, totalCo2Saved } = useMemo(() => {
        const completed = claims.filter(c => c.status === 'completed');
        return {
            totalKgSaved: completed.reduce((sum, c) => sum + (c.socialImpact?.wasteReduction || 0), 0),
            totalCo2Saved: completed.reduce((sum, c) => sum + (c.socialImpact?.co2Saved || 0), 0)
        };
    }, [claims]);

    const dashboardStats = [
        { label: "TOTAL PENYELAMATAN", value: `${totalKgSaved.toLocaleString('id-ID', { maximumFractionDigits: 1 })} kg`, unit: "", subValue: `${claims.filter(c => c.status === 'completed').length} Klaim`, trend: "up", icon: Leaf, color: "text-green-600", bg: "bg-green-100", target: 'impact' },
        ...
        { label: "JEJAK KARBON (C02)", value: `-${totalCo2Saved.toLocaleString('id-ID', { maximumFractionDigits: 1 })}`, unit: "Kg", subValue: "-Real", trend: "down", icon: Globe, color: "text-teal-600", bg: "bg-teal-100", target: 'impact' },
