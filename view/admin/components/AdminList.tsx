...
export const AdminList: React.FC<AdminListProps> = ({ users = [], setUsers, currentUser }) => {
    // Derive admin list from globalUsers filtered by admin roles
    const admins: AdminUser[] = useMemo(() => {
        const adminUsers = users.filter(u => u.role === 'admin_manager' || u.role === 'super_admin');
        if (adminUsers.length === 0) {
            return [{
                id: currentUser?.id || '1',
                name: currentUser?.name || 'Admin',
                email: currentUser?.email || 'admin@foodai.com',
        ...
        return adminUsers.map(u => ({
            ...
            name: u.name,
            email: u.email,
            role: u.role as 'super_admin' | 'admin_manager',
            permissions: u.role === 'super_admin' ? ['all'] : ['moderation', 'distribution'],
    ...
    const handleAddAdmin = async (e: React.FormEvent) => {
        ...
        if (isEditingAdmin && newAdminForm.id) {
            userData.id = newAdminForm.id;
            try {
                await db.upsertUser({ ...userData, id: newAdminForm.id } as UserData);
                if (setUsers) {
                    setUsers(prev => prev.map(u => u.id === newAdminForm.id ? { ...u, ...userData } : u));
                }
    ...
    const handleDeleteAdmin = async (id: string) => {
        if (confirm('Apakah anda yakin ingin menghapus admin ini?')) {
            // Remove the admin role by setting them to 'receiver' or deleting
            if (setUsers) {
                            ...
                            {admins.map(admin => (
                                <div key={admin.id} className="flex items-center justify-between p-3 bg-stone-50 dark:bg-stone-900 rounded-xl border border-stone-100 dark:border-stone-800">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="font-bold text-sm text-stone-900 dark:text-white">{admin.name} {admin.id === currentUser?.id && <span className="text-orange-500 text-xs">(You)</span>}</p>
