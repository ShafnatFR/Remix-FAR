...
const SkeletonFoodCard: React.FC<{ layoutMode: 'list' | 'grid' }> = ({ layoutMode }) => (
    <div className={`bg-white dark:bg-stone-900 rounded-[1.5rem] border border-stone-100 dark:border-stone-800 p-4 flex ${layoutMode === 'list' ? 'flex-row h-32 md:h-40 gap-4 md:gap-6' : 'flex-col gap-3'}`}>
        <Skeleton className={`shrink-0 ${layoutMode === 'list' ? 'w-28 md:w-36 h-full' : 'w-full aspect-square'} rounded-2xl`} />
    ...
    return (
        <div className="p-6 md:p-8 max-w-5xl mx-auto pb-32">
            ...
            <div className="space-y-6">
                {isLoading ? (
                    <div className={`grid gap-3 md:gap-5 ${layoutMode === 'grid' ? 'grid-cols-2 md:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1'} animate-in fade-in duration-500`}>
                        {[1, 2, 3, 4, 5, 6].map(i => (
                            <SkeletonFoodCard key={i} layoutMode={layoutMode} />
                        ))}
                    </div>
                                    ...
                                    <div className={`shrink-0 rounded-xl overflow-hidden bg-stone-100 relative ${layoutMode === 'list' ? 'w-28 md:w-36 h-full' : 'w-full aspect-square'}`}>
                                        ...
                                        <div className="absolute top-2 right-2 z-10">
                                            <button
                                                ...
                                                className={`p-1.5 rounded-lg backdrop-blur-md shadow-sm border transition-all active:scale-90 ${savedIds.has(item.id)
